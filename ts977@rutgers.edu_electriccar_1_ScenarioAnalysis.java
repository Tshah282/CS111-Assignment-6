...
public class ScenarioAnalysis {
    ...
    public void computeCO2EmissionsAndCost () {
        for(Vehicle t : getVehicles()){
            if(t.getFuel().getType()==2){
                double milesKWH = t.getFuel().getUsage();
                double KWhCharge = t.getFuel().getKWhPerCharge();
                double ePrice = getElectricityPrice()/100;
                int leaseYears = t.getLease().getNumberOfMonths()/12;
                int totMiles = t.getLease().getMileageAllowance() * leaseYears;
                double totLeaseCost = t.getLease().getDueAtSigning() + (t.getLease().getMonthlyCost()* 
                t.getLease().getNumberOfMonths());
                double totCO2Emission = ((totMiles/milesKWH) * KWhCharge) * (Fuel.CO2EMITTED_GENERATEmWh/1000) * .45;
                t.setCO2Emission(totCO2Emission);
                t.setFuelCost((totMiles/milesKWH) * (ePrice * KWhCharge));
                t.setTotalCost(totLeaseCost + t.getFuelCost()+ t.getOtherCost());
            }
            else if (t.getFuel().getType() ==1){
                int leaseYears = t.getLease().getNumberOfMonths()/12;
                int totMiles = t.getLease().getMileageAllowance() * leaseYears;
                double totLeaseCost = t.getLease().getDueAtSigning() + (t.getLease().getMonthlyCost()* 
                t.getLease().getNumberOfMonths());
                double totFuelCost = totMiles/ t.getFuel().getUsage() * this.getGasPrice();
                double totCO2Emission = totMiles/ t.getFuel().getUsage() * Fuel.CO2EMITTED_GASCOMBUSTION;
                t.setCO2Emission(totCO2Emission);
                t.setFuelCost(totFuelCost);
                t.setTotalCost(totFuelCost+ totLeaseCost + t.getOtherCost());
            }
        }
